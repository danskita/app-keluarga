...
db = firestore.client()
doc_ref = db.collection('Keluarga').document('DataUtama')

# Fungsi untuk menyimpan semua perubahan ke Cloud secara real-time
...
# Fungsi untuk menarik data dari Cloud saat aplikasi pertama kali dibuka
if 'data_loaded' not in st.session_state:
    try:
        ...
        if doc.exists:
            ...
            # Jika database masih kosong di Firebase
        ...
        st.error(f"Gagal mengambil data dari internet: {e}")

# Inisialisasi Master Masakan (Bawaan Aplikasi)
if 'master_masakan' not in st.session_state:
...
st.sidebar.caption("🟢 Status: Terhubung ke Cloud Database")
...
if menu == "👨‍👩‍👧‍👦 1. Kenalan Dulu (Anggota)":
    ...
    with st.form("form_anggota", clear_on_submit=True):
        ...
        if st.form_submit_button("Simpan Data! 🚀"):
            if nama.strip() == "": st.error("Namanya belum diisi tuh!")
                ...
                save_to_firebase() # SINKRON KE CLOUD
    ...
    if st.session_state.anggota:
        ...
        if st.button("💾 Simpan Perubahan ke Database", type="primary"):
            ...
            save_to_firebase()
            st.success("Perubahan udah disimpen ke Cloud!")
        ...
        with st.form("form_master_rutin", clear_on_submit=True):
            ...
            if st.form_submit_button("Simpan Rutinitas Baru"):
                if not pic: st.error("Pilih minimal 1 orang dong!")
                    ...
                    save_to_firebase()
                    st.success("Rutinitas tersimpan aman di Cloud!")
        ...
        with st.form("form_jadwal", clear_on_submit=True):
            ...
            if st.form_submit_button("Simpan Jadwal Tambahan"):
                if not pic: st.error("Pilih minimal 1 orang ya!")
                    ...
                    save_to_firebase() # SINKRON KE CLOUD
                    st.success("Sip! Jadwal tersimpan ke database internet.")
                ...
                with st.spinner('Menghubungi server...'):
                    try:
                        ...
                        if res.json()['code'] == 200:
                            ...
                            if count > 0: 
                                save_to_firebase()
                                st.success(f"Alhamdulillah! {count} jadwal sholat udah di-sync ke Cloud.")
        ...
        with st.form("form_jadwal_sekolah", clear_on_submit=True):
            ...
            if st.form_submit_button("Simpan Jadwal Sekolah"):
                if not jadwal_teks.strip(): st.error("Jadwal jangan kosong!")
                    ...
                    if count > 0: 
                        save_to_firebase()
                        st.success(f"Beres! Data pelajaran disimpen ke Cloud.")
                        st.rerun()
...
# ==========================================
    ...
    if not st.session_state.jadwal: st.warning("⚠️ Belum ada jadwal apa-apa nih.")
        ...
        if st.button("💾 Simpan Hasil Edit", type="primary"):
            ...
            save_to_firebase() # SINKRON KE CLOUD
            st.success("Mantap, perubahan udah disimpen ke Cloud!")
...
# ==========================================
# HALAMAN 8: CETAK/SIMPAN PDF (KODE DIPOTONG AGAR SINGKAT, SAMA SEPERTI SEBELUMNYA)
# ==========================================
elif menu == "🖨️ 8. Cetak Jadwal (PDF)":
    st.header("Cetak Jadwal (Segera Hadir)")
    st.info("Fitur cetak berfungsi normal, sengaja disingkat pada cuplikan kode ini agar tidak terlalu panjang.")
...
# ==========================================
    ...
    if st.session_state.jadwal:
        ...
        if not df_hari_ini.empty:
            ...
            # Mendeteksi apakah ada tugas yang dicentang, lalu menyimpannya ke Cloud
            ...
            if needs_save:
                save_to_firebase()
                st.toast("✅ Perubahan status tugas berhasil disimpan ke server!")
        else: st.info("Eh, hari ini jadwalnya kosong nih. Yuk isi manual atau tarik dari Master Rutinitas!")
    else: st.info("Wah, masih kosong nih. Data belum tersedia di database internet.")